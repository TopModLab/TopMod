...
DLFLScriptEditor::DLFLScriptEditor(DLFLObjectPtr obj, QWidget *parent, Qt::WindowFlags f)
	: QWidget(parent)
	, dlfl_module(nullptr), dlfl_dict(nullptr)
	, mEchoing(true)
	, pathPython(""), mTabWidth(3), mAddToPath(".")
{
...
void DLFLScriptEditor::executeCommand()
{
		...
		if (Py_IsInitialized())
		{
			...
			for (int i = 0; i < cmdList.size(); i++)
			{
				...
				if (rstring != nullptr)
				{
					...
					if (resultObject1 != nullptr && resultObject2 != nullptr)
					{
	...
	{ // enter when command.isEmpty() == true
		command = QString("");
		result = QString("");
	}
...
void DLFLScriptEditor::PyInit()
{
	...
	if (Py_IsInitialized())
	{
		...
		if (dlfl_module != nullptr)
			dlfl_dict = PyModule_GetDict(dlfl_module);

		if (dlfl_module != nullptr && dlfl_dict != nullptr)
		{
...
void DLFLScriptEditor::loadDLFLModule(QString newPath)
{
	if (!syspath_append(newPath.toLocal8Bit().constData()))
		return;
	if (dlfl_module == nullptr && Py_IsInitialized())
	{
...
void DLFLScriptEditor::execFile()
{
	...
	QString filename = QFileDialog::getOpenFileName(
		this, tr("Exec File"),
		"$HOME", tr("Python Files (*.py);;All Files (*)"),
	...
	if (!filename.isEmpty())
	{
		...
		if (Py_IsInitialized() && main_dict != nullptr)
		{
...
void DLFLScriptEditor::saveFile()
{
	QString filename = QFileDialog::getSaveFileName(
		this, tr("Save Python File"),
		"myscript", tr("Python Files (*.py);;All Files (*)"),
		0, QFileDialog::DontUseSheet);
