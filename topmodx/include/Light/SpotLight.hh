...
public:
	...
	// Default constructor
	SpotLight() : Light(), radius(5.0), cutoff(0.5)
	{
	}

	// 1 argument constructor
	SpotLight(const Vector3d pos) : Light(pos), radius(5.0), cutoff(0.5)
	{
	}

	// Copy constructor
	SpotLight(const SpotLight& sl) : Light(sl), radius(sl.radius), cutoff(sl.cutoff)
	{
	}
