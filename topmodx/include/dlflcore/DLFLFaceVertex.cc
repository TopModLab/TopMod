...
DLFLFaceVertex::DLFLFaceVertex(bool bf)
	: uID(suLastID), vertex(nullptr)
	, normal(), color(1), texcoord(), backface(bf), index(0)
	, epEPtr(nullptr), fpFPtr(nullptr), fvpNext(nullptr), fvpPrev(nullptr)
	, fvtType(FVTNormal)
	, auxcoords(), auxnormal()
{
...
DLFLFaceVertex::DLFLFaceVertex(DLFLVertexPtr vptr, DLFLEdgePtr eptr, bool bf)
	: uID(suLastID), vertex(vptr)
	, normal(), color(1), texcoord(), backface(bf), index(0)
	, epEPtr(eptr), fpFPtr(nullptr), fvpNext(nullptr), fvpPrev(nullptr)
...
DLFLFaceVertex::DLFLFaceVertex(const DLFLFaceVertex& dfv)
	: uID(dfv.uID), vertex(dfv.vertex)
	, normal(dfv.normal), color(dfv.color), texcoord(dfv.texcoord)
	, backface(false), index(dfv.index)
	, epEPtr(dfv.epEPtr), fpFPtr(dfv.fpFPtr)
	, fvpNext(nullptr), fvpPrev(nullptr)
...
bool areEqual(DLFLFaceVertexPtr fvptr1, DLFLFaceVertexPtr fvptr2)
{
	...
	// If either pointer is nullptr returns false unless both are nullptr in which
	// case it returns true
	if (fvptr1 == nullptr)
	{
		if (fvptr2 == nullptr) return true;
		return false;
	}
	else if (fvptr2 == nullptr)
	{
		return false;
	}
	else
	{
		// Both pointers are not nullptr
...
{
	DLFLFaceVertexPtr fvpn = next(), fvpp = prev(), fvp = nullptr;
	...
	if (fvpp == fvpn)
	{
		if (!fvpp->isWingedCorner())
			...
			fvp = fvpn;
		// WARNING! If the code reaches this point, this function will return nullptr
...
{
	// Add self to Edge if there is a nullptr DLFLFaceVertex pointer in the Edge
	if (epEPtr) epEPtr->setNullFaceVertexPtr(this);
}

void DLFLFaceVertex::deleteSelfFromEdge()
{
	// Set the DLFLFaceVertexPtr of Edge which refers to this DLFLFaceVertex to nullptr
