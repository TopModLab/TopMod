...
{
	if (fvpV1 != nullptr && fvpV2 != nullptr) midpoint = 0.5 * (fvpV1->getVertexCoords() + fvpV2->getVertexCoords());
}

// Update the edge normal - average of normals at the 4 corners adjacent to this edge
void DLFLEdge::updateNormal(void)
{
	if (fvpV1 != nullptr && fvpV2 != nullptr)
	{
...
void DLFLEdge::reverse(void) // PRIVATE
{
	...
	//fvpV1->setEdgePtr(nullptr);
	fvpV1 = fvpV1->next(); fvpV1->setEdgePtr(this);
	//fvpV2->setEdgePtr(nullptr);
...
// Get face-vertex pointer belonging to give face. Returns nullptr if not found
...
DLFLFacePtr DLFLEdge::getOtherFacePointer(DLFLFacePtr fptr)
{
	...
	// If given face pointer is not adjacent to this edge, returns nullptr
