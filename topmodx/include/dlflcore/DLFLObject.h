...
public:
	...
	~DLFLObject()
	{
		...
		if (mDirname) { delete[] mDirname; mDirname = nullptr; }
	};
	...
	{
		if (index >= vertex_list.size()) return nullptr;
	...
	DLFLFacePtrArray createFace(const Vector3dArray& verts, DLFLMaterialPtr matl = nullptr, bool set_type = false);

	DLFLFaceVertexPtr createPointSphere(const Vector3d& v, DLFLMaterialPtr matl = nullptr);
