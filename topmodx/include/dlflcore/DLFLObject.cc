...
bool DLFLObject::edgeExists(DLFLVertexPtr vptr1, DLFLVertexPtr vptr2)
{
	...
	if (vptr1->getEdgeTo(vptr2) != nullptr) edgeexists = true;
...
DLFLFacePtrArray DLFLObject::createFace(const Vector3dArray& verts, DLFLMaterialPtr matl, bool set_type)
{
	...
	if (matl == nullptr) matl = firstMaterial();
	...
	// Assume that head1 and head2 are not nullptr
...
DLFLFaceVertexPtr DLFLObject::createPointSphere(const Vector3d& v, DLFLMaterialPtr matl)
{
	...
	DLFLFaceVertexPtr fvp = new DLFLFaceVertex(vp, nullptr);
	fvp->addSelfToVertex();
...
void DLFLObject::vertexTrace(uint vertex_index)
{
	...
	DLFLVertexPtr vertexptr = nullptr;
...
{
	// Find a vertex with the given vertex id. Return nullptr if none exists
	DLFLVertexPtrList::iterator first = vertex_list.begin(), last = vertex_list.end();
	DLFLVertexPtr sel = nullptr;
	while (first != last)
	{
		if ((*first)->getID() == vid)
		{
...
{
	// Find an edge with the given edge id. Return nullptr if none exists
	//DLFLEdgePtrList::iterator first = edge_list.begin(), last = edge_list.end();
	DLFLEdgePtr sel = nullptr;
	/*while ( first != last ) {
	if( (*first)->getID() == eid ) {
	sel = (*first);
	break;
...
{
	// Find a face with the given face id. Return nullptr if none exists
	//DLFLFacePtrList::iterator first = face_list.begin(), last = face_list.end();
	DLFLFacePtr sel = nullptr;
	/*while ( first != last ) {
	if ( (*first)->getID() == fid ) {
	sel = (*first);
	break;
...
{
	// Find a face vertex with the given face vertex id. Return nullptr if none exists
	DLFLFacePtrList::iterator first = face_list.begin(), last = face_list.end();
