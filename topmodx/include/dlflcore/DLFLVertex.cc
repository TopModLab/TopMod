...
DLFLEdgePtr DLFLVertex::getEdgeTo(DLFLVertexPtr vp)
{
	...
	DLFLEdgePtr ep, retep = nullptr;
	DLFLFaceVertexPtrList::iterator first, last;
	DLFLFaceVertexPtr fvp = nullptr, ofvp;
	first = fvpList.begin(); last = fvpList.end();
	while (first != last)
	{
		fvp = (*first); ++first;
...
DLFLFaceVertexPtr DLFLVertex::getFaceVertexInFace(DLFLFacePtr fp)
{
	...
	// belongs to given face, return nullptr
	DLFLFaceVertexPtrList::iterator first, last;
	DLFLFaceVertexPtr fvp, retfvp = nullptr;
	first = fvpList.begin(); last = fvpList.end();
	if (fvpList.size() == 1)
		retfvp = (*first);
	else
...
DLFLFaceVertexPtr DLFLVertex::getFaceVertexWithPrev(DLFLVertexPtr vp)
{
	...
	// If there are more than 1 and none of them satisfies the condition, returns nullptr
	DLFLFaceVertexPtrList::iterator first, last;
	DLFLFaceVertexPtr fvp, retfvp = nullptr;
	first = fvpList.begin(); last = fvpList.end();
	if (fvpList.size() == 1)
...
DLFLFaceVertexPtr DLFLVertex::getBackFaceVertex(void)
{
	...
	// If no such FaceVertex is found, returns nullptr
	DLFLFaceVertexPtrList::iterator first, last;
	DLFLFaceVertexPtr fvp, retfvp = nullptr;
	first = fvpList.begin(); last = fvpList.end();
	while (first != last)
