...
DLFLEdgePtr insertEdgeAltCoFacial(
	DLFLObjectPtr obj,
...
DLFLEdgePtr insertEdgeAltNonCoFacial(
	DLFLObjectPtr obj,
	DLFLFaceVertexPtr fvptr1, DLFLFaceVertexPtr fvptr2,
	...
	for (int j = 0; j < faces.size(); ++j)
	{
		...
		for (int k = 0; k < size; ++k)
		{
			...
			if (edgeExists(svp, evp) == false)
			{
				...
				if (sfvp == nullptr) sfvp = svp->getFaceVertexWithNext(svp_next);

				efvp = evp->getFaceVertexWithPrev(evp_prev);
				if (efvp == nullptr) efvp = evp->getFaceVertexWithNext(evp_next);

				// Check to make sure we have non nullptr FaceVertex pointers
				if (sfvp != nullptr && efvp != nullptr)
				{
