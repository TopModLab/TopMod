...
DLFLFace::DLFLFace(DLFLMaterialPtr mp)
	: head(nullptr), matl_ptr(mp), ftType(FTNormal), auxcoords(), auxnormal(), centroid(), normal(), flags(0)
{
	assignID();
...
DLFLFace::DLFLFace(const DLFLFace& face)
	: uID(face.uID), head(nullptr), matl_ptr(face.matl_ptr), ftType(face.ftType),
...
void DLFLFace::deleteVertexPtr(DLFLFaceVertexPtr dfvp)
{
	...
	dfvp->next() = dfvp; dfvp->prev() = dfvp; dfvp->setFacePtr(nullptr);
}
...
{
	DLFLFaceVertexPtr closest = nullptr;
...
vector<int> DLFLFace::vertexWalk() const
{
	...
	if (head)
	{
		...
		if (current->getVertexPtr() != nullptr)
