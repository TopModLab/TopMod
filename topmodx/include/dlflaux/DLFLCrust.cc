...
void createCrust(const DLFLObjectPtr obj, double thickness, bool uniform)
{
	...
	crustfp1.resize(crust_num_old_faces, nullptr);
	crustfp2.resize(crust_num_old_faces, nullptr);
...
void cmMakeHole(DLFLObjectPtr obj, DLFLFacePtr fp, bool cleanup)
{
	...
	if (fp1 != nullptr && fp2 != nullptr)
	{
		...
		crustfp1[index] = crustfp2[index] = nullptr; // These face pointers are no longer valid
	...
	else cout << "nullptr pointers found" << endl;
...
void createWireframeWithSegments(DLFLObjectPtr obj, double thickness, int numSides)
{
	...
	for (int i = 0; i < 2 * num_original_edges; i++)
	{
		...
		edge_connect_fparray.push_back(nullptr);
	}
	...
	while (num<numVerts)
	{
		...
		{
			vparrayOrdered.push_back(nullptr);
		}
		if (fvparray.size() == 1)
		{
			num_ch_faces = 1;
	...
	while (el_first != el_last)
	{
		...
		{
			if (edge_connect_fparray[count] != nullptr && edge_connect_fparray[count + 1] != nullptr)
			{
