...
DLFLEdgePtr HalfEdgePair::last_connected1 = nullptr;
DLFLEdgePtr HalfEdgePair::last_connected2 = nullptr;
...
void multiConnectFaces(
	DLFLObjectPtr obj, DLFLFacePtrArray fparray, double scale_factor,
...
void createSponge(DLFLObjectPtr obj, double thickness, double collapse_threshold_factor)
{
	...
	fplist1.resize(num_old_faces, nullptr); fplist2.resize(num_old_faces, nullptr);
	...
	while (fl_first != fl_last && num_faces < num_old_faces)
	{
		...
		for (int i = 0; i < (int)eparray1.size(); ++i)
		{
			...
			if (eplist1[edgeindex] == nullptr) eplist1[edgeindex] = ep2;
