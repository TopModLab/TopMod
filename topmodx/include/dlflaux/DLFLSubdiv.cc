...
void pentagonalSubdivide2(DLFLObjectPtr obj, double scale_factor)
{
	...
	{
		if (fvplist1[i] != nullptr && fvplist2[i] != nullptr)
			insertEdge(obj, fvplist1[i], fvplist2[i]);
		else
			cout << "nullptr pointers found! i = " << i << " "
...
void honeycombSubdivide(DLFLObjectPtr obj)
{
	...
	fvplist1.resize(num_old_edges, nullptr);
	fvplist2.resize(num_old_edges, nullptr);
	...
	while (fl_first != fl_last && num_faces < num_old_faces)
	{
		...
		delete[] midpoints; midpoints = nullptr;
		...
		if (fvplist1[edgeindex] == nullptr) fvplist1[edgeindex] = fvp2;
...
bool dooSabinSubdivide(DLFLObjectPtr obj, bool check/*, QProgressDialog *progress*/)
{
	...
	// Reserve and create num_old_edges entries in the 2 temporary lists
	eplist1.resize(num_old_edges, nullptr); eplist2.resize(num_old_edges, nullptr);
	...
	for (int i = 0; i < num_old_edges; ++i)
	{
		...
		if (eplist1[i] != nullptr && eplist2[i] != nullptr)
		{
...
void root4Subdivide(DLFLObjectPtr obj, double a, double twist)
{
	...
	vl_first = obj->beginVertex(); vl_last = obj->endVertex();
	DLFLEdgePtr * iedges = nullptr; // Edges indident on a vertex
	DLFLEdgePtr iedge = nullptr; // Incident edge
	int numiedges = 0;
	Vector3d mp, psum, np;
...
void catmullClarkSubdivide(DLFLObjectPtr obj)
{
	...
	psarray.reserve(num_faces); psarray.resize(num_faces, nullptr);
	fl_first = obj->beginFace(); fl_last = obj->endFace();
	...
	fvplist.resize(numconn, nullptr); vplist.resize(numconn, nullptr);
...
// Star subdivision
// Doug
void starSubdivide(DLFLObjectPtr obj, double offset)
{
...
// Fractal
// Doug
void fractalSubdivide(DLFLObjectPtr obj, double offset)
{
	DLFLFacePtrList::iterator fl_first, fl_last;
...
void dual1264Subdivide(DLFLObjectPtr obj, double sf)
{
	...
	for (int i = 0; i < num_old_edges; ++i)
	{
		if (eplist1[i] != nullptr && eplist2[i] != nullptr)
		{
			...
			cout << "nullptr pointers found! i = " << i << " " << eplist1[i] << " -- " << eplist2[i] << endl;
	}
}
...
void subdivideFace(DLFLObjectPtr obj, DLFLFacePtr faceptr, bool usequads)
{
	...
	if (usequads)
	{
		...
		endpoints.resize(edges.size(), nullptr);
